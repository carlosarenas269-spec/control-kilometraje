...
if st.session_state.etapa_idx == 0:
    ...
    if es_admin:
        with st.expander("⚙️ Administración de Catálogos y Reportes"):
            ...
            with col_m2:
                st.subheader("2. Carga Masiva con Mapeo Manual")
                    ...
                    try:
                        ...
                        if not df_preview.empty:
                            if tipo_carga == "Clientes con Sucursales":
                                st.markdown("##### 🗺️ Mapea las columnas de tu Excel:")
                                columnas_disponibles = list(df_preview.columns)
                                
                                c_map1, c_map2 = st.columns(2)
                                with c_map1:
                                    col_cli = st.selectbox("Columna de Cliente / ID:", columnas_disponibles)
                                    col_dir = st.selectbox("Columna de Sucursal / Nombre:", columnas_disponibles)
                                    col_cont = st.selectbox("Columna de Contacto:", [None] + columnas_disponibles)
                                with c_map2:
                                    col_tel = st.selectbox("Columna de Teléfono:", [None] + columnas_disponibles)
                                    col_lat = st.selectbox("Columna de LATITUD:", columnas_disponibles)
                                    col_lon = st.selectbox("Columna de LONGITUD:", columnas_disponibles)
                                
                                if st.button("📥 Procesar e Importar Clientes con Mapeo"):
                                    nueva_db = {}
                                    lista_c = []
