...
# ==========================================
    ...
    with col_e3_1:
        ...
        if st.button("Registrar Cierre de Ruta", key="btn_cierre_ruta"):
            if km_final_in < st.session_state.get("km_salida_cliente", 0.0):
                ...
                # Activamos la bandera para mostrar el botón de nueva ruta
    ...
    with col_e3_2:
        ...
        if st.session_state.get("ruta_guardada", False):
            ...
            # Botón para regresar a la Etapa 1 y capturar un nuevo registro
            if st.button("🔄 Iniciar Nuevo Registro de Ruta", type="primary"):
                # Limpiamos las variables de horas y datos dinámicos del viaje anterior
                ...
                # Movemos el índice de pestaña a la Etapa 1
                st.session_state.etapa_idx = 0
                st.rerun()
        else:
