...
def obtener_hora_mexico():
    ...
    # Zona horaria de México central (UTC-6)
...
def cargar_catalogos_de_disco():
    """Carga los catálogos desde el disco de forma 100% segura, fusionando sin borrar."""
    if os.path.exists(ARCHIVO_CATALOGOS):
        try:
            ...
            if "Unidades" in xl.sheet_names:
                ...
                if unidades_disco:
                    for u in unidades_disco:
                        if u not in st.session_state.lista_unidades:
                            st.session_state.lista_unidades.append(u)
            ...
            if "Operadores" in xl.sheet_names:
                ...
                if operadores_disco:
                    for op in operadores_disco:
                        if op not in st.session_state.lista_operadores:
                            st.session_state.lista_operadores.append(op)
                ...
                if not df_c.empty:
                    for _, row in df_c.iterrows():
                        ...
                        if num_cli and num_cli.lower() != 'nan':
                            if num_cli not in st.session_state.lista_clientes:
                                st.session_state.lista_clientes.append(num_cli)
                            if num_cli not in st.session_state.db_clientes:
                                st.session_state.db_clientes[num_cli] = {}
                                
                            st.session_state.db_clientes[num_cli][suc_nombre] = {
                                ...
                                "latitud": float(row.get("latitud", 25.844412)) if pd.notna(row.get("latitud", 25.844412)) else 25.844412,
                                "longitud": asegurar_longitud_negativa(row.get("longitud", -100.293652) if pd.notna(row.get("longitud", -100.293652)) else -100.293652)
                            }
        except Exception as e:
...
# 1. Definición robusta de listas predeterminadas base
...
# Cargar y fusionar datos desde disco
if "catalogos_cargados" not in st.session_state:
    cargar_catalogos_de_disco()
    if not st.session_state.lista_unidades:
        st.session_state.lista_unidades = list(UNIDADES_DEFAULT)
    if not st.session_state.lista_operadores:
        st.session_state.lista_operadores = list(OPERADORES_DEFAULT)
...
if st.session_state.etapa_idx == 0:
    ...
    if es_admin:
        with st.expander("⚙️ Administración de Catálogos y Reportes"):
            ...
            with col_m2:
                st.subheader("2. Carga Masiva (Excel Flexible)")
                tipo_carga = st.selectbox("Selecciona qué contiene tu Excel:", ["Lista de Operadores", "Lista de Unidades", "Clientes con Sucursales"], key="tipo_carga_excel")
                ...
                if archivo_excel is not None:
                    if st.button("📥 Procesar e Importar Excel"):
                        try:
                            ...
                            if not df.empty:
                                if tipo_carga == "Lista de Operadores":
                                    col_op = df.columns[1] if len(df.columns) > 1 else df.columns[0]
                                    for _, row in df.iterrows():
                                        val_op = str(row.get(col_op, row.iloc[0]))
                                        if val_op and val_op.lower() != 'nan' and val_op not in st.session_state.lista_operadores:
                                            st.session_state.lista_operadores.append(val_op)
                                    guardar_catalogos_en_disco()
                                    st.success("✅ Operadores importados correctamente.")
                                    st.rerun()
                                elif tipo_carga == "Lista de Unidades":
                                    col_uni = df.columns[1] if len(df.columns) > 1 else df.columns[0]
                                    for _, row in df.iterrows():
                                        val_uni = str(row.get(col_uni, row.iloc[0]))
                                        if val_uni and val_uni.lower() != 'nan' and val_uni not in st.session_state.lista_unidades:
                                            st.session_state.lista_unidades.append(val_uni)
                                    guardar_catalogos_en_disco()
                                    st.success("✅ Unidades importadas correctamente.")
                                    st.rerun()
                                else:
                                    for _, row in df.iterrows():
                                        nc = "S/N"
                                        ...
                                        for col in df.columns:
                                            ...
                                            if "cliente" in c_lower or "num" in c_lower or "id" in c_lower:
                                                nc = str(row[col])
                                            elif "nombre" in c_lower or "sucursal" in c_lower or "direccion" in c_lower:
                                                nd = str(row[col])
                                        
                                        if nc == "S/N" and len(df.columns) > 0:
                                            nc = str(row.iloc[0])
                                        if nd == "Principal" and len(df.columns) > 1:
                                            nd = str(row.iloc[1])
                                        ...
                                        if nc and nc.lower() != 'nan':
                                            ...
                                            for col in df.columns:
                                                if "lat" in str(col).lower():
                                                    try: lat_val = float(row[col])
                                                    except: pass
                                                elif "lon" in str(col).lower():
                                                    try: lon_val = float(row[col])
                                                    except: pass
                                    ...
                                    guardar_catalogos_en_disco()
                                    st.success("✅ Base de clientes importada correctamente.")
                                    st.rerun()
                            else:
            ...
            with col_m3:
                ...
                st.markdown("---")
                if st.button("🔄 Restaurar Catálogos Predeterminados"):
                    st.session_state.lista_unidades = list(UNIDADES_DEFAULT)
                    st.session_state.lista_operadores = list(OPERADORES_DEFAULT)
                    st.session_state.lista_clientes = list(CLIENTES_DEFAULT)
                    st.session_state.db_clientes = dict(DB_CLIENTES_DEFAULT)
                    guardar_catalogos_en_disco()
                    st.success("✅ Catálogos restaurados con éxito.")
                    st.rerun()
