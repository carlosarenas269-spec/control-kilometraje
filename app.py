                            ...
                            if not df.empty:
                                if tipo_carga == "Lista de Operadores":
                                    ...
                                    for _, row in df.iterrows():
                                        val_op = str(row.get(col_op, row.iloc[0])).strip()
                                        if val_op and val_op.lower() != 'nan':
                                            nuevos_ops.append(val_op)
                                    if nuevos_ops:
                                        st.session_state.lista_operadores = nuevos_ops
                                        guardar_catalogos_en_disco()
                                        st.success("✅ Operadores actualizados e importados correctamente.")
                                        st.rerun()
                                elif tipo_carga == "Lista de Unidades":
                                    col_uni = df.columns[1] if len(df.columns) > 1 else df.columns[0]
                                    nuevas_unidades = []
                                    for _, row in df.iterrows():
                                        val_uni = str(row.get(col_uni, row.iloc[0])).strip()
                                        if val_uni and val_uni.lower() != 'nan':
                                            nuevas_unidades.append(val_uni)
                                    if nuevas_unidades:
                                        st.session_state.lista_unidades = nuevas_unidades
                                        guardar_catalogos_en_disco()
                                        st.success("✅ Unidades actualizadas e importadas correctamente.")
                                        st.rerun()
                                else:
                                    # Carga inteligente de clientes y múltiples sucursales con latitudes/longitudes
                                    nueva_db = {}
                                    lista_c = []
                                    for _, row in df.iterrows():
                                        nc = "S/N"
                                        ...
                                        for col in df.columns:
                                            ...
                                            val_c = row[col]
                                            if pd.notna(val_c):
                                                if any(k in c_low for k in ["cliente", "num", "id", "no_cliente"]):
                                                    nc = str(val_c).strip()
                                                elif any(k in c_low for k in ["nombre", "sucursal", "direccion", "planta", "destino"]):
                                                    nd = str(val_c).strip()
                                                elif "contacto" in c_low:
                                                    contacto_val = str(val_c).strip()
                                                elif any(k in c_low for k in ["telefono", "tel", "cel"]):
                                                    tel_val = str(val_c).strip()
                                                elif "lat" in c_low:
                                                    try: lat_val = float(val_c)
                                                    except: pass
                                                elif "lon" in c_low or "long" in c_low:
                                                    try: lon_val = float(val_c)
                                                    except: pass

                                        # Respaldo por posición si no encontró nombres de columnas claros
                                        if nc == "S/N" and len(df.columns) > 0 and pd.notna(row.iloc[0]):
                                            nc = str(row.iloc[0]).strip()
                                        if nd == "Principal" and len(df.columns) > 1 and pd.notna(row.iloc[1]):
                                            nd = str(row.iloc[1]).strip()
                                        if len(df.columns) > 4 and pd.notna(row.iloc[4]):
                                            try: lat_val = float(row.iloc[4])
                                            except: pass
                                        if len(df.columns) > 5 and pd.notna(row.iloc[5]):
                                            try: lon_val = float(row.iloc[5])
                                            except: pass
                                    ...
                                    if lista_c:
                                        ...
                                        st.session_state.db_clientes = nueva_db
                                        guardar_catalogos_en_disco()
                                        st.success("✅ Base de clientes, sucursales y coordenadas importadas correctamente.")
                                        st.rerun()
                                    else:
                                        st.warning("No se pudieron extraer clientes del archivo.")
                            else:
