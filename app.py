...
# MÓDULO DE INCIDENCIAS EN SIDEBAR
...
with st.sidebar.expander("Registrar Incidencia / Parada"):
    ...
    if st.button("🚨 Guardar Incidencia"):
        ahora_inc = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        data_inc = {
...
if st.session_state.etapa_idx == 0:
    ...
    if es_admin:
        ...
        # ==========================================
        # VISUALIZADOR Y ELIMINADOR DE HISTORIAL EN PANTALLA PRINCIPAL
        # ==========================================
        st.markdown("---")
        ...
        if os.path.exists(ARCHIVO_REGISTROS):
            try:
                ...
                if not df_historial.empty:
                    ...
                    # ==========================================
                    # NUEVO: ELIMINAR REGISTROS DEL HISTORIAL
                    # ==========================================
                    st.markdown("---")
                    st.subheader("🗑️ Eliminar Registro del Historial")
                    
                    # Crear opciones legibles para identificar la fila (ej. Fila 0 - [Fecha] - [Operador] - [Unidad])
                    opciones_filas = []
                    ...
                    if st.button("❌ Eliminar Registro Seleccionado", type="primary"):
                        try:
                            # Extraer el índice numérico de la cadena seleccionada (ej. "Fila 0 | ...")
