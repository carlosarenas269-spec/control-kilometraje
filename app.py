...
def cargar_catalogos_de_disco():
    """Carga los catálogos desde el disco si el archivo existe, respetando la base de datos actual."""
    if os.path.exists(ARCHIVO_CATALOGOS):
        try:
            ...
            if "Clientes" in xl.sheet_names:
                ...
                nueva_db = st.session_state.db_clientes.copy()
                lista_c = st.session_state.lista_clientes.copy()
                
                for _, row in df_c.iterrows():
                    cli_nombre = str(row["Cliente"])
                    if cli_nombre not in lista_c:
...
# Cabecera principal con Título y Logo a la derecha
    ...
    if es_admin:
        with st.expander("⚙️ Administración de Catálogos y Reportes"):
            ...
            with col_m4:
                ...
                st.markdown("Descarga el archivo con todos los registros de viajes capturados en la nube o PC.")
